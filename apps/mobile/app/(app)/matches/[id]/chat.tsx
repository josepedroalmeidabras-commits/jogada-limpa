...
export default function MatchChatScreen() {
  ...
  const load = useCallback(async () => {
    ...
    const [m, msgs] = await Promise.all([
      fetchMatchById(id),
      fetchMatchMessages(id),
    ]);
    setMatch(m);
    setMessages(msgs);
    setLoading(false);
    void markMatchChatRead(id);
  }, [id]);
  ...
  useEffect(() => {
    if (!id) return;
    const unsub = subscribeMatchMessages(id, (msg) => {
      setMessages((prev) => {
  ...
  async function handleSend() {
    ...
    setSending(true);
    const r = await sendMatchMessage(id, text);
    setSending(false);
    if (r.ok) setText('');
  }
        ...
        {loading ? (
          ...
          >
            <Text style={styles.banner}>
              💬 Chat partilhado entre as duas equipas
            </Text>
            {messages.length === 0 ? (
                  ...
                  <View key={m.id}>
                    ...
                    >
                      ...
                      >
                        ...
                        <Text
                          style={[styles.time, isMine && styles.timeMine]}
                        >
